...
class MCPServiceManager:
    ...
    def __init__(self, mcp_service_url: str = XHS_MCP_SERVICE_URL):
        ...
        if getattr(sys, 'frozen', False) and hasattr(sys, '_MEIPASS'):
            # PyInstaller 环境
            # 资源文件被打包在 app/xhs_mcp
            self.xhs_mcp_dir = Path(sys._MEIPASS) / "app" / "xhs_mcp"
            
            # 调试日志
            logger.info(f"DEBUG: sys.frozen=True")
            logger.info(f"DEBUG: sys._MEIPASS={sys._MEIPASS}")
            logger.info(f"DEBUG: xhs_mcp_dir={self.xhs_mcp_dir}")
        else:
            # 开发环境
            # mcp_service_manager.py 在 app/utils/ 下
            # xhs_mcp 在 app/xhs_mcp/ 下
            # 所以是 ../../xhs_mcp
            self.project_root = Path(__file__).resolve().parent.parent
            self.xhs_mcp_dir = self.project_root / "xhs_mcp"
            
        logger.debug(f"MCP 服务目录: {self.xhs_mcp_dir}")
