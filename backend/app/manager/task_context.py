...
class Task_Manager_Context:
    ...
    def __init__(self, task_id: str, **kwargs):
        ...
        # 文件存储路径
        # 优先使用 APP_DATA_DIR (用户数据目录)，确保持久化
        self._data_dir = APP_DATA_DIR / "manager" / "data"
    ...
    def create_new_meta(self, **kwargs):
        ...
        # 处理 task_type：如果是枚举类型，转换为字符串
        task_type = kwargs.get("task_type")
        if hasattr(task_type, "value"):  # 检查是否为枚举
        ...
        self.meta = {
            ...
            "task_type": task_type,  # 任务类型（已转换为字符串）
